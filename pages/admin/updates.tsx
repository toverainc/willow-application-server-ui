    ...
    return (
        <Card sx={{ margin: 1, width: "300px" }}>
            ...
            <CardContent sx={{ padding: 1 }}>
                ...
                <List dense={true} >
                    {release.assets.map(a => <ListItem key={a.id} secondaryAction={
                        a.local
                            ? <Tooltip style={{ boxShadow: "none" }} title='Delete this release' enterTouchDelay={0}><IconButton edge="end" aria-label="delete" onClick={e => handleDelete(a)}><DeleteIcon /></IconButton></Tooltip>
...
const Updates: NextPage = () => {
    const { data, error } = useSWR<Release[]>('/api/releases/?refresh=true')
