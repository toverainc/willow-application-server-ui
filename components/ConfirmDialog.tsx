...
export function ApplyConfigDialog({ open, onClose, client }: { open: boolean, client?: Client, onClose: (event: any) => void }) {
    ...
    return <ConfirmDialog
        open={open}
        onClose={onClose}
        message={client ? `Clicking confirm will apply current config to "${client.label || client.hostname}"` : `Clicking confirm will apply current config to all devices.`}
        onConfirm={onConfirm} />
}
...
export function ApplyNvsDialog({ open, onClose, client }: { open: boolean, client?: Client, onClose: (event: any) => void }) {
    ...
    return <ConfirmDialog
        open={open}
        onClose={onClose}
        message={client ? `Clicking confirm will apply current NVS config to "${client.label || client.hostname}"` : `Clicking confirm will apply current NVS config to all devices.`}
        onConfirm={onConfirm} />
}